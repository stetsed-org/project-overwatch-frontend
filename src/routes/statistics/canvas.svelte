...
{#if !formSubmitted}
  <form on:submit={fetchData}>
    ...
    <button type="submit" class="btn btn-md sm:btm-sm md:btm-md lg:btn-lg"
      >Submit</button
    >
  </form>
{:else}
  <div class="container mx-auto p-4 justify-center">
    <button
      ...
      on:click={handleChangeName}>Change Information</button
    >
    {#if responseData.length > 0}
      {#if formSubmitted}
        <div
          class="bg-[url('https://cdn.selfhostable.net/map.png')] w-[1000px]"
        >
          <canvas
            bind:this={canvas}
            {width}
